    ...
    <>
      ...
      <section className="border-t border-content/10 py-16">
        <div className="mx-auto max-w-4xl px-5 sm:px-6">
          {posts.length === 0 ? (
            <p className="text-content-dim">No posts yet. Check back soon!</p>
          ) : (
                ...
                return (
                  <AnimateIn key={post.slug} delay={i * 0.08}>
                    {isPublished ? (
                      <Link
                        href={`/blog/${post.slug}`}
                        className="group block border-b border-content/[0.06] py-8 transition-colors first:pt-0 hover:bg-content/[0.02] -mx-4 px-4 rounded-lg"
                      >
                        <PostRow post={post} />
                      </Link>
                    ) : (
                      <div className="border-b border-content/[0.06] py-8 opacity-40 first:pt-0 -mx-4 px-4">
                        <PostRow post={post} />
                      </div>
                    )}
                  </AnimateIn>
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-6">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-4">
          ...
          <ArrowRight
            size={16}
            className="mt-1.5 flex-shrink-0 text-content-faint opacity-0 transition-all group-hover:opacity-100 group-hover:translate-x-1 group-hover:text-accent"
          />
        </div>
        <p className="mt-1.5 text-sm leading-relaxed text-content-dim line-clamp-2">
          {post.excerpt}
        </p>
        <span className="mt-2 inline-block font-jetbrains text-[11px] text-content-faint">
          {post.readTime}
        </span>
      </div>
    </div>
